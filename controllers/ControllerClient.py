...
def obtener_clientes_por_usuario(skip: int = 0, limit: int = 100, busqueda: str = None, servicio: ClienteService = Depends(), usuario_actual: Usuario = Depends(obtener_usuario_actual)):
    clientes = servicio.obtener_clientes_por_usuario(usuario_actual.id, skip=skip, limit=limit, busqueda=busqueda)
    return clientes

@router.get("/cambios-clientes", response_model=List[CambioClienteResponse])
def obtener_cambiosClientes(skip: int = 0, limit: int = 100, servicio: ClienteService = Depends(), usuario_actual: Usuario = Depends(obtener_usuario_actual)):
...
def cambiar_estado_cliente(cliente_id: int, nuevo_estado: str, servicio: ClienteService = Depends(), usuario_admin: Usuario = Depends(requerir_rol_admin)):
    return servicio.cambiar_estado_cliente(cliente_id, nuevo_estado, usuario_admin.id)
